    ...
    case Lexer::Lexeme::ReservedWord:
    {
        if(node->children.size() > 0)
        {
            ...
            /*else if(node->expression.second == "switch")
            {
                ...
                auto switchVar = vars.find(args[0].second);
                
                for(auto i = node->children[1]->children.begin(); i < node->children[1]->children.end(); i++)
                {
                    if((*i)->expression.first == Lexer::Lexeme::ReservedWord && (*i)->expression.second == "case")
                    {
                        if(IsEqual(switchVar->second->GetData(), GetReturn(*i, vars)).second == "true")
                        {
                            for(auto j = i + 1; j < node->children[1]->children.end(); j++)
                            {
                                if((*j)->expression.first == Lexer::Lexeme::ReservedWord && ((*j)->expression.second == "case" || (*j)->expression.second == "default"))
                                    break;
